...
<%--
	MVc : 요청 => 데이터 갱신 => 뷰로 전송
			서버를 거칠 때 마다 새로운 JSP가 생성
				=> init ==> service ===> 출력
				=> 새로 고침 ==> 출력된 내용(메모리에서 삭제) ===> destroy
								===========================
								  전부 지워졌다가 다시 생성
	MVVN : View 패턴
		=============================
 ...
 --%>
    ...
    <div class="container" id="app2">
        <div class="row">
            <input type="button" value="한식" class="btn btn-sm btn-primary"
                @click="select(1)"
            >
            <input type="button" value="양식" class="btn btn-sm btn-danger"
                @click="select(2)"
            >
            <input type="button" value="중식" class="btn btn-sm btn-success"
                @click="select(3)"
            >
            <input type="button" value="일식" class="btn btn-sm btn-info"
                @click="select(4)"
            >
            <input type="button" value="분식" class="btn btn-sm btn-warning"
                @click="select(5)"
            >
        </div>
